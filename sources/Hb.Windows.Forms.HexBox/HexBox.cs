...
namespace Hb.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		public void InsertBytes(string fill, long count, int mod)
		{
			if (!CanInsert()) return;
			
			if (mod==0)
			sourc = ConvertHexToBytes(fill);
			if (mod == 1)
				sourc = Encoding.ASCII.GetBytes(fill);
			if (sourc == null)
			{
			...
			if (_selectionLength > 0)
				_byteProvider.DeleteBytes(_bytePos, _selectionLength);
			long coefficient = count / sourc.LongLength;
			long remains = count - (coefficient * sourc.LongLength);
			byte[] buffer = new byte[count];
			for (long i = 0; i < coefficient; i ++)
		...
		/// <param name="count"></param>
		/// <param name="mod"></param>
		public void FillBytes(string fill, long count, int mod)
		{
		...
		{
			int[] formatStruct = { 16, 10, 8 };
			// Ensure endByte isn't > length of array.
			endByte = Math.Min(_byteProvider.Length - 1, endByte);

			Color lineInfoColor = (this.InfoTextColor != Color.Empty) ? this.InfoTextColor : this.ForeColor;
			Brush brush = new SolidBrush(lineInfoColor);
			Brush back = new SolidBrush(InfoBackColor);
			...
			for (int i = 0; i < maxLine; i++)
			{
				...
				string info;
				if (_lineInfoFormat == 0)
					info = firstLineByte.ToString(_hexStringFormat, System.Threading.Thread.CurrentThread.CurrentCulture);
				else
					info = System.Convert.ToString(firstLineByte, formatStruct[_lineInfoFormat]);
		...
		{
			Brush brush = new SolidBrush(this.InfoTextColor);
			Brush back = new SolidBrush(InfoBackColor);
		...
		void PaintColumnSeparator(Graphics g)
		{
			...
			{
				var pen = new Pen(new SolidBrush(this.InfoTextColor), 1);
		...
		void PaintHex(Graphics g, long startByte, long endByte)
		{
			...
			Brush selBrush = new SolidBrush(_selectionTextColor);
		...
		void PaintColumnInfo(Graphics g, byte b, Brush brush, int col)
		{
			...
			if (_lineInfoFormat == 0)
				...
				sB= System.Convert.ToString(col, formatStruct[_lineInfoFormat]);
		...
		{
			Brush brush = new SolidBrush(GetDefaultForeColor());
			Brush brushStr = new SolidBrush(_hexStringTextColor);
			Brush brushStrBack = new SolidBrush(_hexStringBackColor);
			Brush selBrush = new SolidBrush(_selectionTextColor);
			Brush selBrushBack = new SolidBrush(_selectionBackColor);
		...
		void UpdateRectanglePositioning()
		{
            ...
            int requiredWidth = _recBorderRight + _recBorderLeft;
		...
		[DefaultValue(typeof(Color), "White"), Browsable(false)]
		...
		/// </summary>
		[EditorBrowsable(state: EditorBrowsableState.Never)]
		[DefaultValue(typeof(Color), "Black"), Browsable(false)]
		...
		public int LineInfoFormat
		{
			get { return _lineInfoFormat; }
			set
			{
				if (_lineInfoFormat == value || value > 2 || value < 0) 
					return;
		...
		/// </summary>
		[Category("HexAppearance"), DefaultValue(typeof(Color), "WhiteSmoke"), Description("Gets or sets the background color for the disabled HexBox")]
		public Color BackColorDisabled { get; set; } = Color.WhiteSmoke;
		...
		public Color InfoTextColor
		{
			...
			set { _infoTextColor = value; Invalidate(); }
		...
		/// </summary>
		[DefaultValue(typeof(Color), "White"), Category("HexAppearance"), Description("Gets or sets the line info color. When this property is null, then ForeColor property is used.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Gray"), Category("HexAppearance"), Description("Gets or sets the Hex View  color. When this property is null, then ForeColor property is used.")]
		public Color HexViewTextColor
		{
		...
		/// </summary>
		[DefaultValue(typeof(Color), "White"), Category("HexAppearance"), Description("Gets or sets the Hex View back color.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("HexAppearance"), Description("Gets or sets the Hex string view text color")]
		public Color HexStringTextColor
		{
			get { return _hexStringTextColor; }
			set { _hexStringTextColor = value; Invalidate(); }
		}
		Color _hexStringTextColor = Color.Black;

		/// <summary>
		/// Gets or sets the  Hex string view back color.
		/// </summary>
		[DefaultValue(typeof(Color), "White"), Category("HexAppearance"), Description("Gets or sets the Hex string view back color")]
		public Color HexStringBackColor
		{
			get { return _hexStringBackColor; }
			set { _hexStringBackColor = value; Invalidate(); }
		}Color _hexStringBackColor = Color.White;

		/// <summary>
		/// Gets or sets the background color for the selected bytes.
		/// </summary>
		[DefaultValue(typeof(Color), "Blue"), Category("HexAppearance"), Description("Gets or sets the background color for the selected bytes.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "White"), Category("HexAppearance"), Description("Gets or sets the foreground color for the selected bytes.")]
		public Color SelectionTextColor
		{
			get { return _selectionTextColor; }
			set { _selectionTextColor = value; Invalidate(); }
		} Color _selectionTextColor = Color.White;

		/// <summary>
		/// Gets or sets the foreground color for the highlighted bytes.
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("HexAppearance"), Description("Highlight foreground color.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Yellow"), Category("HexAppearance"), Description("Highlight background color.")]
		...
		/// </summary>
		/// <param name="hex">the hex string separated by ' '. For example: "0A 0B 0C" or "0A0B0C"</param>
